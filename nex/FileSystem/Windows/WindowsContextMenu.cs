...
{
    //TODO: After using this menu refresh DVC
    public class WindowsContextMenu : NativeWindow
    {
        #region Fields
        ...
        protected override void WndProc(ref Message m)
        {
            ...
            if (_oContextMenu != null &&
                ...
                ((int)ShellHelper.HiWord64(m.WParam) & (int)MFT.SEPARATOR) == 0 &&
                ((int)ShellHelper.HiWord64(m.WParam) & (int)MFT.POPUP) == 0)
            {
                string info = string.Empty;

                if (ShellHelper.LoWord64(m.WParam) == (int)CMD_CUSTOM.ExpandCollapse)
        ...
        /// Get the PIDLs
        /// </summary>
        /// <param name="arrFSI">Array of DirectoryInfo</param>
        /// <returns>Array of PIDLs</returns>
        protected IntPtr[] GetPIDLs(FileSystemInfo[] arrFSI, string parentDirectory)
        {
            if (null == arrFSI || 0 == arrFSI.Length)
            {
                return null;
            }

            IShellFolder oParentFolder = GetParentFolder(parentDirectory);
            if (null == oParentFolder)
            {
                return null;
            }
        ...
        public void ShowContextMenu(FileSystemInfo[] infos, Point pointScreen, string parentDirectory)
        {
            // Release all resources first.
            ReleaseAll();
            _arrPIDLs = GetPIDLs(infos, parentDirectory);
            this.ShowContextMenu(pointScreen);
        }
